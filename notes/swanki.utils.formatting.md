...
## 2026.05.14 - parse_chapter_key + chapter_number_spoken (drives new bookend format)

Two new helpers extracted from `humanize_chapter_slug` to support the rewritten lecture bookend format. The bookend now reads `"This lecture is posted as: <base humanized>, <chapter number spoken>, <slug humanized>. Let's begin chapter <N>, <slug>."` (and the symmetric end form), per direct user spec — listener wanted the audio to read the citation key "exactly as written" with the chapter number rendered as `"o one"` for `01` rather than the prior `"Chapter 3: history of computers hardware. From Hamming, Art Doing Science, 2020."` form which dropped the leading zero.
