...
## 2026.06.03 - Orphan pre-first-chapter spill bug (Alcamo CH05)

**The break.** Alcamo Schaum's Microbiology CH05 failed `audit_coverage` with exactly one unsolved problem, `MAT-CH5-2-1` (item 1 of the second Matching set), while `problem-pairings.yaml` showed THREE pairings keyed `MAT-CH5-2-1`: the real forward statement (0 solutions) plus two contaminants carrying the orphan Chapter-3 answer tail (one with the Completion answers `nanometer 2. mycoplasmas …`, one with the orphan Matching line `d 2. a 3. a …`).

**Root cause — forward section span bleed.** The packed CH05 answer key is book page 329; its MinerU OCR is the last clean-md page and it *begins* with Chapter 3's spilled answer tail as headerless `# Completion` / `# Matching` blocks BEFORE the first `# Chapter 4` header (orphan spill). `_section_spans` ends each forward section at the next `_SECTION_OR_BACK_OF_BOOK` anchor, which only recognized `#{1,3} Answers` and `#{1,3} Chapter N` as back-of-book boundaries. The forward Matching set-2 span therefore ran past the chapter's real items, straight through the orphan `# Completion` / `# Matching` blocks, and stopped only at `# Chapter 4`. `_MATCHING_ITEM` then enumerated the orphan's `1. nanometer …` and `1. d 2. a …` lines as two extra forward Matching items, both with item number 1, colliding into `MAT-CH5-2-1` and stealing the slot from the real item. `_partition_back_of_book` was NOT the culprit — it already discards everything before `chapter_matches[0]`, so the orphan never entered the back-of-book map; the contamination was entirely on the forward-enumeration side.

**The fix — one regex alternative.** Added the back-of-book *section* headers to `_SECTION_OR_BACK_OF_BOOK`: `#{1,3}\s+(?:Multiple Choice|Matching|True/False|Completion)\s*$`. A real `#` section header now terminates a forward span, so set-2 ends at the orphan `# Completion` and the orphan answer lines are never enumerated as forward items. Safe because the in-chapter forward dividers are always bare-text (`Matching.` with a trailing period, matched by the separate `Matching\.\s+\S` alternative), never `#` headers — verified empirically: every `#{1,3} Section` header in the real CH05 document belongs to the answer key. No change to same-chapter page-spill handling or the repeated-section (two-Matching) handling.

**CH05 before → after for `MAT-CH5-2-1`.** Before: 3 pairings under that ID, the real one with 0 solutions → `CoverageError(1 unsolved)`. After: 1 pairing, pairs to `(d)` (set-2 body item 1 = `d`); all 10 set-1 and 10 set-2 Matching items pair with a single solution; `audit_coverage` passes with 0 unsolved.

**Regression.** Offline harness over the real MinerU dumps: CH01 still 49/49, CH03 still fully pairs (51/51), CH05 now 0-unsolved — all pass `audit_coverage(allow_unsolved=False)`.

Fixture added: `schaum_orphan_spill.md` — Chapter 5 forward content (two `Matching.` sets) followed by an answer key that LEADS with a foreign chapter's headerless `# Completion` + `# Matching` spill, then `# Chapter 4`, `# Chapter 5` (two `# Matching` sets), `# Chapter 6`. `TestOrphanPreFirstChapterSpill` asserts the orphan is discarded from the partition (`["4","5","6"]`), no duplicate Matching IDs, `MAT-CH5-2-1 -> "(d) Thermophile"`, every Matching item single-solution, and that no problem statement carries the orphan `nanometer` answer text.
