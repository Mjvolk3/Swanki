...
- [x] Fixed `abs_set_chapter_titles.py` idempotency check to also compare chapter `start`/`end` boundaries (0.5s tolerance) so re-renders that change audioFile durations don't leave stale chapter markers in the ABS UI. Caught after Hamming v2 rerun showed Ch1 and Ch2 visually grouped (Ch1 marker ended 132.8s short of the actual Ch01 audio). Per-book log line gains "boundaries shifted" action label. Hamming Lecture chapter boundaries also re-POSTed manually via one-shot to fix the live state [[scripts.abs_set_chapter_titles#20260514---bounds-aware-idempotency-check-catches-stale-chapter-boundaries-after-re-render]]

- [x] Rewrote lecture bookend format per user spec: chapter form now reads `"This lecture is posted as: Hamming, Art Doing Science, 2020, o three, history of computers hardware. Let's begin chapter three, history of computers hardware."` and end mirrors. Driven by new `parse_chapter_key` and `chapter_number_spoken` helpers in `swanki/utils/formatting.py`. Old form ("Chapter 3: history of computers hardware. From Hamming, Art Doing Science, 2020.") dropped the leading zero and reordered components. Hamming Ch1-5 bookends regenerated + restitched + re-uploaded to ABS [[swanki.utils.formatting#20260514---parse_chapter_key--chapter_number_spoken-drives-new-bookend-format]] [[swanki.audio._common#20260514---generate_bookend_audio-rewritten-for-chapter-form-theme-8-follow-up]]

- [x] Fixed `restitch_from_chunks` to honor the boundary-fix knobs (`chunk_pause_ms`, `chunk_tail_trim_ms`, `chunk_crossfade_ms`, `gain_match_target_dbfs`, `section_pause_ms`) by persisting them in the chunk manifest at `write_chunk_manifest` time and reading them back on restitch. Surfaced after the surgical bookend regen produced back-to-back chunks with zero inter-chunk silence (hardcoded `chunk_crossfade_ms=0` and omitted everything else). Lecture/summary/reading callers updated; new tests cover the regression and caller-override precedence. Hamming Ch1-5 also re-restitched with proper knobs and re-uploaded to ABS [[swanki.audio._common#20260515---postprocessor-knobs-persisted-in-chunk_manifest-restitch-reads-them-back]]

- [x] Content-aware inter-chunk silence: `chunk_text_paragraphs` now returns `(chunk_text, boundary_type)` tuples ("paragraph" vs "sentence"), `combine_audio_with_section_pauses` accepts a `chunk_pause_ms_by_boundary` map keyed by boundary type, and the manifest persists per-chunk boundary types so `restitch_from_chunks` reproduces the content-aware silences. Defaults: paragraph=1100ms, sentence=500ms (replacing the prior uniform 700ms). Listener design: pure-silence concat-time gaps stay (vs. Fish pause tags) for guaranteed-clean output, but the duration now reflects the source-text break type [[swanki.audio._common#20260515---content-aware-inter-chunk-silence-paragraph-vs-sentence]]

- [x] Mirrored the chapter-aware bookend format to summary + reading: `generate_bookend_audio` now uses the "This {audio_type} is posted as: ... Here is the {audio_type} of chapter N, slug" / "This concludes the {audio_type} of chapter N, slug, which is posted as: ..." frame for summary + reading on chapter inputs, matching the lecture treatment. Extracted `build_bookend_text` as a pure function so the rules are unit-testable; 9 new tests cover all chapter / non-chapter × lecture / summary / reading × start / end combinations [[swanki.audio._common#20260516---build_bookend_text-extracted--chapter-form-mirrored-to-summary--reading]]

## 2026.05.17

- [x] Implemented the Hamming Ch1 audio two-track plan: feed-forward fixes (RC1 sentinel-protects `---SECTION_BREAK---` from `expand_acronyms_for_tts` so Fish never speaks "section break"; RC2 deterministic `reading_coverage_ratio` Pass-2 completeness guard; RC3 per-paper prosody addendum; RC4 row-wise table linearization in `humanize_latex`) plus a reusable `swanki.audio.surgical.regenerate_and_restitch` tool + CLI, and surgically repaired the live Hamming Ch1 reading audio (chunks 0/31/37/53/54 re-TTS'd, other 53 byte-identical) including the dropped page 4->5 sentence; also fixed the unpaginated `zot.children()` bug that hid the source PDF from `/zotero-annotations` [[plan.hamming-chapter-1-audio-two-track-fixes.2026.05.17]]
- [ ] Planned a durable two-track fix for the 6 Hamming Ch1 orange Zotero annotations (4 root causes): Track A sentinel-protects `---SECTION_BREAK---` from `expand_acronyms_for_tts`, adds a deterministic Pass-2 completeness guard, a row-wise table-linearization rule, and a per-paper prosody addendum; Track B is a reusable surgical single-chunk re-TTS + `restitch_from_chunks` tool to patch the live reading manifest without full regen — also fixed the unpaginated `zot.children()` bug that hid the source PDF from `/zotero-annotations` [[plan.hamming-chapter-1-audio-two-track-fixes.2026.05.17]]

## 2026.05.18

- [x] Implemented exact chunk-time mapping + `/audio-fix-from-annotations` skill: extracted a shared `_accumulate_timeline` from `combine_audio_with_section_pauses` (audio + per-chunk offsets from one traversal, byte-identical, proven by an independent-oracle + determinism test), `restitch_from_chunks` now writes a measured `chunk_timeline.json` sidecar, added `chunk_time_window`/`time_to_chunk`/`chunk_time_window_abs` (exported), new `scripts/abs_bookmarks.py` (ABS `/api/me`, fixture-tested) and a `.claude/skills/audio-fix-from-annotations` skill (default orange, one review gate); 115 audio/timeline tests green incl. measured-not-predicted + sidecar-rewrite-on-restitch [[plan.exact-chunk-time-mapping-audio-fix-from-annotations.2026.05.18]]
- [x] Shipped solution-manual mode foundation: `mode=solution_manual` whole-document override, models + agents + chapter-index extractor + problem-set pipeline (enumerate, pair, resolve, plan, generate, audit), Hydra presets, and Schaum's chapter-pack helper [[swanki.pipeline.problem_set]]
