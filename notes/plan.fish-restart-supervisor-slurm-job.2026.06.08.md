...
1. **Does the Layer-1 retry re-register the voice reference after a respawn? RESOLVED: no re-registration needed — the reference survives via disk.** `ensure_fish_speech_reference` runs once at setup (`pipeline.py:2159`) and `_tts_fish_speech` sends only `reference_id`, so a respawned Fish has empty *in-memory* reference state. But Fish's `/v1/references/add` persists the reference to `references/<id>/` ON DISK (`fish_speech/inference_engine/reference_loader.py:add_reference`), and the sbatch bind-mounts that dir from the host (`$FISH_REPO/references:/app/references`, confirmed populated: `british-prof/`, `hamming/`). On the cold process `load_by_id` re-reads + re-encodes from disk (cache-miss is a re-encode, not a failure). So the supervisor needs NO re-registration and `_common.py` stays untouched.
2. **Is `/v1/health` 200 sufficient on respawn? RESOLVED: yes — the same proven gate as the initial launch.** The initial launch already serves TTS off this exact `/v1/health` gate, and the first post-respawn TTS's cold reference re-encode fits inside the client's 1800s read timeout (`httpx.Timeout(1800.0, connect=60.0)`). `SWANKI_FISH_RESPAWN_SETTLE` (5s) adds margin before re-bind. No extra grace step added; revisit only if a live respawn shows a too-early reconnect.
