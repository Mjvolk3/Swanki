...
- [x] Added swanki-attributed ABS bookmark creation to the core module (`add_bookmark` + `ABSClient.create_bookmark` + `python -m swanki.abs add-bookmark`): file-local time shifted to item-global via the audioFiles walk, notes prefixed `🦢 swanki:`; first use dropped two markers on Hamming CH04 at 1:57 and 14:21 (item-global 39:31 / 51:55) pointing at the live `[break]`/`[long-break]` A/B spots [[swanki.abs.bookmarks]]
