...
- [ ] Scope the binary-codeword TTS reading so ordinary 0/1-only decimals (10, 100, 1000) stop being mangled: trim the self-contradictory `10` example from the BINARY CODEWORDS prompt rule at all four prompt sites (default.yaml, book_voice.yaml, reading.py, summary.py), demote the `verbalize_bit_strings` scrubber to per-paper opt-in (default-off), and add a CH10 codeword regression fixture (the LLM already emits word-form codewords, so default-off has zero CH10 regression) [[plan.scope-binary-codeword-tts.2026.06.06]]
