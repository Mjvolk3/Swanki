...
- [ ] Standardize table/figure audio landmarks: deterministic `Figure: <desc>` / `Table: <desc>` (no number) injected at the markdown layer, full caption verbatim or a ≤1-sentence generated summary when absent, bracketed by real `---SECTION_BREAK---` silence, with a new `table_processor` + `TableSummary` model mirroring image-summaries and table cells never voiced [[plan.reading-table-figure-landmarks.2026.05.31]]
