...
def generate_reading_audio(
    ...
    **tts_kwargs: object,
    ...
    system_prompt = (
        ...
        "3. FIGURE and TABLE LANDMARKS — figures and tables are ALREADY converted to "
        "landmarks of the form 'Figure: <description>' or 'Table: <description>', each "
        "bracketed by ---SECTION_BREAK--- on its own line before and after. PRESERVE each "
        "landmark EXACTLY: keep the literal 'Figure:' / 'Table:' prefix (NO figure or table "
        "number — never add 'Figure 1', 'Table 2', etc.), keep the description verbatim, and "
        "keep both ---SECTION_BREAK--- markers. Do NOT merge a landmark into surrounding "
        "prose and do NOT rephrase its description. NEVER read image URLs, mathpix links, or "
        "alt-text bracket markers — those are not part of a landmark.\n\n"
