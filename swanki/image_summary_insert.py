...
def process_markdown_files(source_dir: str, summary_dir: str, target_dir: str):
    ...
    for md_file in md_files:
        ...
        if not image_urls:
            ...
            with open(os.path.join(target_dir, md_file), "w", encoding="utf-8") as file:
        ...
        for j, image_url in enumerate(image_urls):
            ...
            if summary:
                new_content = new_content.replace(
                    image_url, f"{cleaned_image_url}\n\n{summary}"
                )
            else:
                print(f"No summary found for {cleaned_image_url} in {summary_file}.")
