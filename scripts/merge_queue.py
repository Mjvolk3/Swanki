...
[[scripts.merge_queue]]
https://github.com/Mjvolk3/Swanki/tree/main/scripts/merge_queue.py
...
def _resolve_data_root() -> Path | None:
    """Resolve ``SWANKI_DATA`` (where the queue DB lives) without python-dotenv.

    Order: the ``SWANKI_DATA`` env var first (set by ``.env`` in a normal
    shell); then the repo ``.env`` parsed by hand (a ``*/2`` cron shell has no
    ``SWANKI_DATA`` exported, but the file is always present). Stdlib-only so the
    module keeps its zero-dependency portability contract.
    """
    root = os.environ.get("SWANKI_DATA")
    if root:
        return Path(root)
    env_file = Path(__file__).resolve().parents[1] / ".env"
    if env_file.exists():
        for line in env_file.read_text().splitlines():
            if line.startswith("SWANKI_DATA="):
...
# The DB lives under SWANKI_DATA (Swanki's canonical large/shared data root,
# e.g. /scratch/projects/torchcell-scratch/Swanki_Data). Falls back to a
# per-user cache dir only if SWANKI_DATA cannot be resolved at all (never
# expected in practice).
_DATA_ROOT = _resolve_data_root()
DEFAULT_DB_PATH = (
    (_DATA_ROOT / "dev" / "merge_queue" / "merge_queue.db")
    if _DATA_ROOT is not None
    else Path.home() / ".cache" / "swanki" / "merge_queue" / "merge_queue.db"
)
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Path to the SQLite DB (default: $SWANKI_DATA/dev/merge_queue/merge_queue.db).",
    )
