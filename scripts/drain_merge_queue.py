...
https://github.com/Mjvolk3/Swanki/tree/main/scripts/drain_merge_queue.py
...
REPO_SLUG = "Mjvolk3/Swanki"

# A "free note" is swept to main; scratch and paired module/test/script notes
# are not -- those travel with their source through a worktree. We classify
# explicit dirty paths in Python and `git add` exactly those, rather than rely
# on `git add <pathspec>` (which aborts the whole command if any single
# pathspec matches nothing).
_PAIRED_OR_SCRATCH_PREFIXES = ("scratch.", "swanki.", "scripts.", "tests.")
...
def _worktree_for(main: Path, branch: str, recorded: str | None) -> Path:
    ...
    return main.parent / "Swanki.worktrees" / branch
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Queue DB path (default: $SWANKI_DATA/dev/merge_queue/merge_queue.db).",
    )
    parser.add_argument(
